...
class BattleMenu(menu.Menu):
    def __init__(self, game, monsterList, amount):
        ...
        self.skillState = 0
        # whenever skills of these types are selected, skip the enemy selection phase (since these are self applying)
        self.skipEnemySelection = ["Healing", "Cleanse", "Buff", "Conversion"]
        # whenever a new battle is encountered, assemble a battle troop.
    ...
    def display_again(self):
        ...
        # if the player selected skills, then display all of their skills
    ...
    # draw the enemy on the screen. Hide the health if player is blinded by an enemy.
    ...
    # determine the turn order of battle based on speed.
    ...
    def calculate_turn_order(self):
        ...
        # use this dictionary to generate a list comprised of tuples of the form
    ...
    # occurs when player attempts to attack: this handles the cursor and enemy selection
    def attack(self, skill):
        position_of_enemy1X = self.mid_w-180 + self.offset
        position_of_enemy1Y = self.mid_h-210
        enemyTarget = ""
        # check if the player has enough energy to cast their skill
        if (skill.getEnergy() > self.protag.getCurrEnergy()):
            ...
            self.reinitialize()
            return
        # skip enemy selection if the skill is self-casting, like a heal or buff
        ...
        # select enemy accordingly
            ...
            if (len(self.battleTroop) >= 2):
                ...
                pass
        # if an enemy was selected, calculate the turn order and execute the battle.
    ...
    # Below are functions for ailments
    ...
    def attackTurnOrder(self, enemyTarget, skill):
        ...
        for eachEntity in self.turnList:
            ...
            # -- Handle the player's turn in battle -- END
            else:
                enemy = self.nameToEnemy(
                    eachEntity)
                # choose a random skill from the enemy's skill list
                ...
                # check if protagonist was defeated
                if (self.protag.getCurrHealth() <= 0):
    ...
    # check for player input to adjust the cursor.
    # inputs will have different functionalities depending on flags such as show_skills
