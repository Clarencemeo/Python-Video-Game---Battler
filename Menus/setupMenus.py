...
# handles the levelup menu where players can allocate their stats.
...
# handles the victory menu where the player wins battles
...
class classMenu(menu.Menu):
    ...
    def check_input(self):
        ...
        if self.game.START_KEY:
            self.menuConfirmSound.play()
            # deepcopy to ensure we don't change the original object
            menu.protagonist = copy.deepcopy(self.classList[self.classState])
            self.game.curr_menu = eventMenus.directionsMenu(self.game)
            self.run_display = False
